...
// internal
const MAX_LENGTH = env.getNumber('GEO_MAX_LENGTH', 0.0001);
const MAX_ROTATION = env.getNumber('GEO_MAX_ROTATION', Math.PI / 8);
...
const readShapefile = (path, done) => {
    ...
    const onFeature = ({ done: finished, value: feature }) => {
      const next = error => {
        if (error) {
          ...
          return common.mergeObjects(results);
        } else {
  ...
  shapefile.open(path)
    ...
    .then(finished => done(null, common.mergeObjects(finished))) // handle read finish
